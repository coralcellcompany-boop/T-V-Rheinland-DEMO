...
@Component({
  selector: 'tuv-transition-timeline',
  standalone: true,
  ...
  template: `
    <div class="timeline">
      @for (step of items(); track step.id; let last = $last) {
        ...
        <div class="empty">No transitions yet — create the first action by submitting the certificate.</div>
      }
    </div>
  `,
  styles: [
    `
      ...
      .step {
        ...
        gap: 0.85rem; padding-bottom: 1.4rem;
      }
      .step:not(.last)::before {
        content: ''; position: absolute; left: 19px; top: 36px; bottom: -2px;
        width: 2px; background: linear-gradient(180deg, #cbd5e1, #e2e8f0);
      }
      .bullet {
        width: 38px; height: 38px; border-radius: 50%;
        display: flex; align-items: center; justify-content: center;
        background: var(--bg, #eef2ff); color: var(--fg, #4338ca);
        border: 2px solid #fff; box-shadow: 0 0 0 2px var(--border, #c7d2fe);
        font-size: 1rem;
        z-index: 1;
      }
      ...
      .line1 {
        display: flex; align-items: center; gap: 0.45rem; font-weight: 600; color: #0f172a;
        font-size: 0.95rem;
      }
      .line1 .from { color: #94a3b8; font-weight: 500; }
      .line1 .sep { color: #94a3b8; font-size: 0.7rem; }
      .line2 {
        display: flex; gap: 0.4rem; align-items: center; font-size: 0.8rem; color: #64748b;
        margin-top: 0.15rem;
      }
      .line2 .actor {
        background: #f1f5f9; padding: 0.05rem 0.45rem; border-radius: 999px;
        font-weight: 500; color: #475569;
      }
      .line2 .dot { color: #cbd5e1; }
      .comment {
        margin-top: 0.5rem; padding: 0.55rem 0.75rem; background: #f8fafc;
        border-left: 3px solid #cbd5e1; border-radius: 4px; color: #334155; font-size: 0.85rem;
      }
...
export class TransitionTimeline {
  readonly transitions = input.required<CertificateTransition[]>();
  ...
  protected iconFor(state: number): string {
    const map: Record<number, string> = {
      0: 'pi-pencil',         // Draft
      1: 'pi-send',           // Submitted
      2: 'pi-eye',            // UnderReview
      3: 'pi-clock',          // AwaitingApproval
      4: 'pi-check',          // Approved
      5: 'pi-share-alt',      // ClientSent
      6: 'pi-check-circle',   // ClientAccepted
      7: 'pi-flag',           // ClientRejected
      8: 'pi-times',          // Rejected
      9: 'pi-ban',            // Voided
      10: 'pi-hourglass',     // Expired
      11: 'pi-inbox',         // Archived
    };
    return map[state] ?? 'pi-circle';
  }

  protected toneFor(state: number): string {
    if ([4, 6].includes(state)) return 'success';
    if ([7, 8, 9, 10].includes(state)) return 'danger';
    if ([3].includes(state)) return 'warning';
    if ([1, 2, 5].includes(state)) return 'info';
    return 'neutral';
  }
}
